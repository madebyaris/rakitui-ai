 ...
 */
export function getFrameworkGuidelines(framework) {
 ...
 */
export function buildEnhancedPrompt(context) {
...
OUTPUT: Valid JSON with 3 button designs`;
    ...
    validateOutput: (output) => {
        return Boolean(output &&
            ...
            output.designs.every((d) => d.name && d.html && d.description));
    }
};
/**
 ...
 */
export function getTemplateForComponent(componentType) {
 ...
 */
export function generateDesignNames(componentType) {
    const type = componentType.toLowerCase().trim();
    const names = {
        button: [
